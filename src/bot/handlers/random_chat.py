...
# دریافت متدهای لایه خنثی کش و لاگر از فایل کانفیگ
...
def register_random_chat_handlers(bot: AsyncTeleBot):
    ...
    async def handle_start_random_chat(message):
        ...
        # 🔥 پاتک لوکال ایمپورت منوها
        from src.bot.handlers.private_anon import get_keyboards
        kb_main, kb_search, kb_chatting = get_keyboards()
        
        if status == 'chatting':
            await bot.reply_to(message, f"{EMOJI['caution']['html']} شما در یک چت فعال هستید! اول باید با دکمه زیر چت قبلی رو قطع کنی.", reply_markup=kb_chatting)
            return
        if status == 'searching':
            await bot.reply_to(message, f"{EMOJI['magnifiyer']['html']} شما در صف جستجو هستید...", reply_markup=kb_search)
            return

        if not gender:
            # 💎 استفاده از کلید ['char'] روی دکمه‌ها برای فعال‌سازی رندر پرمیوم متحرک
            ...
            await bot.reply_to(message, f"{EMOJI['caution']['html']} <b>برای استفاده از چت تصادفی ابتدا باید جنسیت خودت رو تعیین کنی:</b>\n(این اطلاعات فقط یک‌بار دریافت میشه و قابل تغییر نیست)", parse_mode="HTML", reply_markup=markup_gender)
            return

        markup_filter = InlineKeyboardMarkup().add(
            InlineKeyboardButton(text=f"{EMOJI['sus']['char']} شانسی و کاملاً رایگان", callback_data="filter_any")
        ).row(
        ...
        await bot.reply_to(message, f"{EMOJI['thunder']['html']} <b>نوع اتصال چت تصادفی رو انتخاب کن:</b>\n{EMOJI['coin']['html']} موجودی فعلی شما: {coins} سکه", parse_mode="HTML", reply_markup=markup_filter)
    ...
    async def handle_set_gender_callback(call):
        ...
        await bot.edit_message_text(f"{EMOJI['crcl_yes']['html']} جنسیت شما ثبت شد. حالا می‌توانی دوباره دکمه 🎲 <b>شروع چت تصادفی</b> را بزنی تا فیلترها باز شوند!", user_id, call.message.message_id, parse_mode="HTML")
    ...
    async def handle_filter_selection_callback(call):
        ...
        search_msg = await bot.send_message(user_id, f"{EMOJI['magnifiyer']['html']} <b>[فیلتر: {filter_text}]</b> در حال جستجو برای کاربر هم‌سطح...", parse_mode="HTML", reply_markup=kb_search)
    ...
    async def handle_cancel_queue(message):
        ...
        await bot.reply_to(message, f"{EMOJI['banned']['html']} با موفقیت از صف جستجو خارج شدی و سکه‌هات برگشت خورد.", reply_markup=kb_main)
    ...
    async def handle_disconnect_chat(message):
        ...
        if partner_id:
            ...
            # 💎 اعمال ترفند ساختار داینامیک ['char'] روی دکمه‌های فیدبک و لایک
            ...
            try:
                await bot.send_message(partner_id, f"{EMOJI['caution']['html']} <b>پارتنر شما چت را قطع کرد.</b>\n{EMOJI['qe']['html']} کیفیت چت چطور بود؟ بهش امتیاز بده (+۱ سکه هدیه):", parse_mode="HTML", reply_markup=kb_main)
                await bot.send_message(partner_id, f"{EMOJI['up']['html']} لطفاً امتیاز خود به پارتنر سابق را در کادر بالا ثبت کنید.", reply_markup=markup_partner)
    ...
    async def handle_rating_callbacks(call):
        ...
        if action == "like":
            ...
            await bot.edit_message_text(f"{EMOJI['crcl_yes']['html']} Meresi! بازخورد مثبتت ثبت شد و حساب شما شارژ گردید.", user_id, call.message.message_id)
        elif action == "dis":
            ...
            await bot.edit_message_text(f"{EMOJI['banned']['html']} ثبت شد. این کاربر وارد لیست سیاه چت تصادفی شما شد و ۱ سکه هدیه دریافت کردی.", user_id, call.message.message_id)