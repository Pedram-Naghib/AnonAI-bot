...
async def send_daily_analytics(bot):
    ...
    try:
        if response and response.text:
                ...
                # 🚨 پاتک طنز: ساخت متن هشدار و چسباندن آن به بالای گزارش
                warning_header = (
                    "🚨 <b>⚠️ Disclaimer ⚠️</b>\n"
                    "<i>متن زیر صرفاً جنبهٔ طنز و فاقد ارزش قانونی دارد! این گزارش توسط هوش مصنوعی Gemini بعد از آنالیز چت‌های گروه ساخته شده است؛ لذا از هرگونه چُس کردن، قهر و ناراحت شدن اکیداً خودداری کنید!</i>\n"
                    "───────────────────\n\n"
                )
        else:
