...
async def send_daily_analytics(bot):
    ...
    try:
        if response and response.text:
                ...
                # 🚨 ست کردن امضای رسمی و نهایی برای گروه اصلی کوداکستان
                warning_header = (
                    "🚨 <b>⚠️ Disclaimer ⚠️</b>\n"
                    "<i>گزارش و روت‌های زیر صرفاً جنبهٔ طنز و سرگرمی دارد و توسط هوش مصنوعی Gemini بعد از جویدنِ چت‌های گروه ساخته شده است. صمیمی‌ترین رفقا هم اینجا روست میشن، پس از هرگونه چُس کردن و ناراحت شدن خودداری کنید و به مصلحت بزرگتر ایمان بیاورید!</i>\n"
                    "<i>همچنین این قابلیت فقط در گروه کوداکستان با مدیریت آیدین کونی در دسترس است.</i>\n"
                    "───────────────────\n\n"
                )
                await bot.send_message(GROUP_CHAT_ID, warning_header, parse_mode="HTML")
        else:
